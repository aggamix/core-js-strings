...
function countVowels(str) {
  ...
  for (let i = 0; i < str.length; i++) {
    // eslint-disable-next-line no-plusplus
    if (arrVowels.includes(str.split('')[i])) counter++;
  }
  return counter;
}
...
function isPalindrome(str) {
  const sortStr = str.replace(/[.,/#!?$%^&*;:{}=\-_`~()\s]/g, '').toLowerCase();
  return sortStr === sortStr.split('').reverse().join('');
}
...
function encodeToRot13(str) {
  ...
  return str
    ...
    .map((item) =>
      item
        .split('')
        .map((i) => arrRot13[arrAlp.indexOf(i)])
        .join('')
    )
    .join(' ');
}
